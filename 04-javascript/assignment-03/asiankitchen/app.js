...
const filterButton = document.querySelector("#buttons");
...
document.addEventListener("DOMContentLoaded", renderMenuItem(menu));
filterButton.addEventListener("click", filteredItems);

function categoryButtons() {
  filterButton.innerHTML = `<button class="btn btn-outline-dark btn-item" data-id="All">All</button>
...
function renderMenuItem(list) {
  ...
  list.forEach((item) => {
    ...
    singleItem.className = "menu-items col-lg-6 col-sm-12";
    singleItem.innerHTML = `<img
    src=${item.img}
    alt=${item.title}
    class="photo"
    />
